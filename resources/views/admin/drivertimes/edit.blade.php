    ...
    <div class="row layout-top-spacing">
        <div class="col">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('admin.drivers-times.update',$driversTime)}}" class="form" method="POST">
                        @csrf
                        @method('PATCH')
                        <div class="form-group">
                            <label for="from" class="font-weight-bold">بداية من:</label>
                            <input type="time" name="from" id="from" class="form-control" value="{{$driversTime['from']}}">
                            <x-error title="from"></x-error>

                        </div>
                        <div class="form-group">
                            <label for="from" class="font-weight-bold">نهاية الي :</label>
                            <input type="time" name="to" id="to" class="form-control" value="{{$driversTime['to']}}">
                            <x-error title="to"></x-error>

                        </div>
                        <div class="form-group">
                            <label for="max" class="font-weight-bold">اقصي عدد</label>
                            <input type="number" name="max" id="max" class="form-control" value="{{$driversTime['max']}}">
